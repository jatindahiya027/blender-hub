...
if (db.versions.length == 0) {
    content.innerHTML = `<div class="uk-alert-danger" uk-alert>
        <p>No Blender installed or registered.<br>Please register the installed Blender or install it newly。</p></div>`
} else {
    let html_data = ""
    let card_html_data = ""
    ...
    for (const key in db.versions) {
        ...
        <p class="uk-text-bolder">folder location: ${db.versions[key].dir}</p>
        <button data-id="${key}" class="uk-button uk-button-primary" onclick="lunch(this)">Start-up</button>
        ...
        onclick="lunch(this)">Start-up</button>
...
selectFolder.addEventListener('click', () => {
    dialog.showOpenDialog(null, {
        ...
        title: 'Register installed Blender'
    }).then(result => {
        if (!result.canceled) {
...
ipcRenderer.on('res_load_dir', (event, ...args) => {
    UIkit.modal(loadingModal).hide().then(() => {
        if (args[0]) {
            ...
            UIkit.notification({ message: 'blender.exe was not found from the specified directory。', status: 'danger' })
        }
    })
})
...
ipcRenderer.on('res_add_database', (event, ...args) => {
    ...
    if (args) {
        ...
        UIkit.notification({ message: 'An error has occurred。ERR: ' + args[1], status: 'danger' })
    }
})

// 実行中のエラー
ipcRenderer.on('run_err', (event, arg) => {
    UIkit.notification({ message: 'An error has occurred。ERR: ' + arg, status: 'danger' })
})

const lunch = (arg) => {
    let id = arg.getAttribute("data-id")
    console.log(id);
    UIkit.notification({ message: 'starting up...', status: 'primary' })
    ...
    UIkit.modal(editModal).hide().then(() => {
        UIkit.modal(loadingModal).show().then(() => {
            ...
            if (saveRes[0]) {
                ...
                UIkit.modal(loadingModal).hide().then(() => {
                    UIkit.notification({ message: 'An error occurred while writing。ERR: '+saveRes[1], status: 'danger' })
                });
            }
        });
    })
    ...
    UIkit.modal(editModal).hide().then(() => {
        UIkit.modal(loadingModal).show().then(() => {
            ...
            if (removeRes[0]) {
                ...
                UIkit.modal(loadingModal).hide().then(() => {
                    UIkit.notification({ message: 'An error occurred while deleting。ERR: '+removeRes[1], status: 'danger' })
                });
            }
        });
    })
